...
import 'package:hadramootleagues/coachinboxscreen.dart';
import 'package:hadramootleagues/coachprofilescreen.dart';
import 'package:hadramootleagues/coachupdates.dart';
...
import 'package:hadramootleagues/requestcoachpage.dart';
import 'package:hadramootleagues/requestpage.dart';
import 'package:hadramootleagues/inbox.dart';
import 'package:hadramootleagues/sendcoachquestion.dart';
import 'package:hadramootleagues/sendquestion.dart';
import 'package:hadramootleagues/teamfilterpage.dart';
...
class _CoachHomePageState extends State<CoachHomePage> {
  ...
  Future<void> _getUserData() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('coaches').doc(userId).get();
      if (doc.exists) {
        setState(() {
          coachName = doc['name'] ?? ''; // إذا كان null، استخدم قيمة افتراضية
          coachClub = doc['club'] ?? '';
          imageUrl = doc['imageUrl'] ?? '';
        });
        print('coachName: $coachName, coachClub: $coachClub'); // تحقق من القيم بعد التعيين
      }
    } catch (e) {
  ...
  Future<void> createData() async {
    ...
    try {
      ...
      DocumentReference docRef = FirebaseFirestore.instance.collection('coaches').doc(userId);
  ...
  Widget build(BuildContext context) {
    ...
    stream: userId != null
    ? FirebaseFirestore.instance.collection('coaches').doc(userId).snapshots()
    ...
    title: Text(
    coachName != null && coachName!.isNotEmpty ? coachName! : "لا يوجد اسم",
    style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black),
    ),
    subtitle: Text(
    coachClub != null && coachClub!.isNotEmpty ? coachClub! : "لا يوجد فريق",
    ...
    context,
    MaterialPageRoute(
    builder: (context) => CoachProfileScreen(userId: userId!),
    ),
    );
    } else if (value == 2) {
    logout(context);
    ...
    String updatedCoachName = data?['name'] != null && data!['name'].isNotEmpty
    ? data['name']
        : "لا يوجد اسم";
    String updatedCoachClub = data?['club'] != null && data!['club'].isNotEmpty
    ? data['club']
        : "لا يوجد فريق";
    ...
    logout(context);
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.home_outlined), label: "التصفح"),
