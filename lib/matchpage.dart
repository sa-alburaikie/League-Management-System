...
class _MatchesScreenState extends State<MatchesScreen> {
  Map<String, TextEditingController> _resultControllers = {};
          ...
          actions: [
            IconButton(
              ...
              onPressed: () async {
                if (_isEditing) {
                  // حفظ التغييرات
                  for (var matchId in _resultControllers.keys) {
                    String result = _resultControllers[matchId]!.text;
                    if (result.isNotEmpty && result.contains('-')) {
                      await FirebaseFirestore.instance.collection('matches').doc(matchId).update({
                        'result': result,
                        'goals1': int.tryParse(result.split('-')[0]) ?? 0,
                        'goals2': int.tryParse(result.split('-')[1]) ?? 0,
                        'isFinished': true,
                      });
                    }
                  }
  ...
  Widget buildMatchRow(Map<String, dynamic> match) {
    ...
    if (!_resultControllers.containsKey(matchId)) {
      _resultControllers[matchId] = TextEditingController(text: match['result'] ?? '');
    }
    ...
    width: 60,
    child: TextField(
    controller: _resultControllers[matchId],
